...
export async function getDataset(env=process.env,fetcher=fetch){
 ...
 for(let offset=0;offset<20000;offset+=1000){const target=new URL('/rest/v1/toilets',url);target.searchParams.set('select','id,name,lat,lng,address,hours,type,retrieved_at');target.searchParams.set('order','id.asc');target.searchParams.set('offset',String(offset));target.searchParams.set('limit','1000');const response=await fetcher(target,{headers:{apikey:key,Authorization:`Bearer ${key}`},signal:AbortSignal.timeout(10000)});if(!response.ok)throw new Error('DATABASE_UNAVAILABLE');const page=await response.json();if(!Array.isArray(page))throw new Error('DATABASE_UNAVAILABLE');for(const t of page){if(Number.isFinite(t.lat)&&Number.isFinite(t.lng)){const {retrieved_at,...row}=t;rows.push(row);if(retrieved_at>retrievedAt)retrievedAt=retrieved_at;}}if(page.length<1000)break;}
